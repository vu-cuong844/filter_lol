...
public class BugOut extends Fillter_Face {
    private List<Mat> gifFrames = new ArrayList<>();
    private int frameIndex = 0;

    public BugOut() {
        loadGif("lib\\library\\filter_picture\\conchim.gif", Imgcodecs.IMREAD_UNCHANGED);
    }
    ...
    public void filtering(Mat src, Mat des) { 
       ...
       for (Rect rect : faceDetections.toArray()) {
           Rect location = new Rect(rect.x, rect.y - rect.height/4 > 0 ? rect.y - rect.height/4 : 0,rect.width,rect.height/4);
